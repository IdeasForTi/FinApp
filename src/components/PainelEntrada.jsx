...
const PainelEntrada = ({ 
  ...
  // FILTRO: Se for Variável, separa Ação de FII. Se for Fixa, mostra Fixa.
  const historicoAtivo = mainTab === 'variavel' 
    ? (historyVariavel || []).filter(item => {
        const isItemFii = item.ticker?.endsWith('11');
    ...
    <div className="space-y-6 text-left">
      ...
      {mainTab === 'variavel' && (
        <div className="bg-slate-900 p-2 rounded-2xl border border-slate-800 flex gap-2">
          <button onClick={() => setIsFiiMode(false)} className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase flex items-center justify-center gap-2 transition-all ${!isFiiMode ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-500'}`}><LineChart size={14}/> Ações</button>
          <button onClick={() => setIsFiiMode(true)} className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase flex items-center justify-center gap-2 transition-all ${isFiiMode ? 'bg-emerald-600 text-white shadow-lg' : 'text-slate-500'}`}><Building2 size={14}/> FIIs</button>
        </div>
      )}
        ...
        {mainTab === 'variavel' ? (
          <div className="space-y-4">
            <div className="flex gap-2 items-end">
              <div className="flex-1">
                ...
                <input name="ticker" value={inputs.ticker || ''} onChange={handleChange} className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 font-mono uppercase outline-none focus:ring-1 focus:ring-blue-500 text-white transition-all" />
              </div>
              <button onClick={buscarDados} disabled={loading} className="bg-blue-600 hover:bg-blue-500 p-3.5 rounded-xl transition-all active:scale-95 shadow-lg">
            ...
            <div className="grid grid-cols-2 gap-3 text-white">
              ...
              <input type="number" name="currentPrice" value={inputs.currentPrice || ''} onChange={handleChange} className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 font-mono outline-none" /></div>
              <div><label className="text-[9px] font-bold text-slate-500 uppercase ml-1">{isFiiMode ? 'Rend. Mensal R$' : 'Div. Yield %'}</label>
            ...
            <div className="grid grid-cols-2 gap-3 text-white">
              {!isFiiMode && (
                ...
                <input type="number" name="eps" value={inputs.eps || ''} onChange={handleChange} className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 font-mono outline-none" /></div>
              )}
              <div className={isFiiMode ? "col-span-2" : ""}><label className="text-[9px] font-bold text-slate-500 uppercase ml-1">VPA</label>
              <input type="number" name="bvps" value={inputs.bvps || ''} onChange={handleChange} className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 font-mono outline-none" /></div>
            </div>

            {!isFiiMode && (
              <div className="grid grid-cols-2 gap-3 text-white">
                <div><label className="text-[9px] font-bold text-slate-500 uppercase ml-1">P/L Atual</label>
                <input type="number" name="peRatio" value={inputs.peRatio || ''} onChange={handleChange} className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 font-mono outline-none" /></div>
          ...
          <div className="space-y-4 text-white">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <input type="number" name="cdiTaxaAnual" value={inputs.cdiTaxaAnual || ''} onChange={handleChange} className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 font-mono outline-none" /></div>
            ...
            <input type="number" name="cdiMeses" value={inputs.cdiMeses || ''} onChange={handleChange} className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 font-mono outline-none" /></div>
            <button onClick={salvarCalculoFixa} className="w-full bg-emerald-600 hover:bg-emerald-500 py-4 rounded-xl font-black text-[10px] uppercase mt-2 flex items-center justify-center gap-2 transition-all active:scale-95 shadow-lg shadow-emerald-900/20 text-white"><History size={14}/> Salvar Simulação</button>
          </div>
        )}
      </section>

      <section className="bg-slate-900 p-6 rounded-2xl border border-slate-800 shadow-xl">
        <h2 className="flex items-center gap-2 text-xs font-black uppercase text-slate-500 mb-4 tracking-widest"><History size={16}/> Histórico recente</h2>
