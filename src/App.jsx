...
const StockApp = () => {
  ...
  const [inputs, setInputs] = useState({ ticker: 'BBAS3', precoAtual: 27.50, dividendo: 2.20, lpa: 5.80, vpa: 32.10, pl: 4.70, crescimento: 10.00 });
  ...
  const buscarDados = async () => {
    ...
    try {
      const response = await fetch(`https://brapi.dev/api/quote/${inputs.ticker.toUpperCase()}?token=sxKPbJ9GaFUK9wPWB3Zjs9&fundamental=true`);
      const data = await response.json();
      if (data.results?.[0]) {
        const stock = data.results[0];
        const novosDados = { ...inputs, precoAtual: stock.regularMarketPrice || inputs.precoAtual, lpa: stock.priceEarnings ? (stock.regularMarketPrice / stock.priceEarnings) : inputs.lpa, vpa: stock.bookValuePerShare || inputs.vpa, pl: stock.priceEarnings || inputs.pl };
  ...
  const dy = inputs.precoAtual > 0 ? (inputs.dividendo / inputs.precoAtual) * 100 : 0;
  const peg = (inputs.crescimento + dy) > 0 ? (inputs.pl / (inputs.crescimento + dy)) : 0;
